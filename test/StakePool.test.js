...
describe("StakePool Contract", function () {
    ...
    beforeEach(async function () {
        ...
        stakePool = await StakePool.deploy(await sttToken.getAddress());
        await stakePool.waitForDeployment();
    });

    describe("초기화", function () {
        it("컨트랙트가 올바르게 초기화되어야 한다", async function () {
            expect(await stakePool.stt()).to.equal(await sttToken.getAddress());
    ...
    describe("Owner 관리", function () {
        it("owner가 아닌 계정은 withdraw를 호출할 수 없어야 한다", async function () {
            await expect(stakePool.connect(user1).withdraw(user1.address, ethers.parseEther("1")))
    ...
    describe("Pool 잔액 조회", function () {
        ...
        it("STT 토큰을 전송한 후 Pool 잔액이 증가해야 한다", async function () {
            ...
            await sttToken.transfer(await stakePool.getAddress(), amount);
    ...
    describe("출금 기능", function () {
        ...
        it("zero address로 출금할 수 없어야 한다", async function () {
            await expect(stakePool.connect(owner).withdraw(ethers.ZeroAddress, ethers.parseEther("1")))
    ...
    describe("이벤트 발생", function () {
        ...
        it("출금 시 Withdrawn 이벤트가 발생해야 한다", async function () {
            await expect(stakePool.connect(owner).withdraw(user1.address, ethers.parseEther("50")))
    ...
    describe("STT 토큰 상호작용", function () {
        it("STT 토큰 잔액을 올바르게 조회할 수 있어야 한다", async function () {
            const balance = await sttToken.balanceOf(await stakePool.getAddress());
        ...
        it("STT 토큰을 Pool에 전송할 수 있어야 한다", async function () {
            ...
            const poolBalance = await sttToken.balanceOf(await stakePool.getAddress());
    ...
    describe("Pool 상태 확인", function () {
        ...
        it("Pool의 잔액이 정확히 반영되어야 한다", async function () {
            ...
            await sttToken.transfer(await stakePool.getAddress(), amount1);
            await sttToken.transfer(await stakePool.getAddress(), amount2);
        ...
        it("출금 후 Pool 잔액이 감소해야 한다", async function () {
            ...
            await sttToken.transfer(await stakePool.getAddress(), initialAmount);
    ...
    describe("에러 처리", function () {
        ...
        it("owner가 아닌 계정은 출금할 수 없어야 한다", async function () {
            await expect(stakePool.connect(user1).withdraw(user2.address, ethers.parseEther("1")))
                .to.be.revertedWithCustomError(stakePool, "OwnableUnauthorizedAccount");
        });
    });
}); 